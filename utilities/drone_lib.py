...
def upload_new_flight_path(device, flight_data, alt_scale=1.0):
    """
...
def move_local(device, x, y, z, duration=1, log=None, cube=True):
    ...
    send_body_frame_velocities(device, y, x, z, duration)
    return
...
def goto_point(device, lat, lon, speed, alt, log=None, timeout=None, wait=None):
    ...
    while device.armed \
        ...
        try:
            if wait is None:
                ...
                if (0.99 <= alt_percent <= 1.01) \
                        and (.99 <= lat_percent <= 1.01) \
                        and (.99 <= lon_percent <= 1.01):
                    time.sleep(1)
                ...
                if timeout is not None:
                    if secs_waited >= timeout:
                        log_activity(f"Goto timeout reached (n = {timeout} seconds).", log)
                        break
            else:
                time.sleep(1)
                secs_waited += 1
                if secs_waited >= wait:
                    log_activity(f"Wait for goto expired (n = {wait} seconds).", log)
